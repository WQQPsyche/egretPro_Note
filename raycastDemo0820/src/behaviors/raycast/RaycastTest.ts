...
export abstract class BaseRaycast extends Behaviour {
    ...
    protected _updateRay() {
        ...
        const ray = this.ray;
        ray.origin.copy(transform.position);//射线起点

        transform.getForward(ray.direction);//获取该节点z轴正方向
    }
}
...
export class RendererRaycast extends BaseRaycast {
    ...
    public onUpdate() {
        const raycastInfo = this.raycastInfo;//射线碰撞信息
        const lineTransform = this._line.transform;
        lineTransform.setLocalScale(1.0);
        raycastInfo.clear();
        raycastInfo.normal = this.normal;//相交的法向量
